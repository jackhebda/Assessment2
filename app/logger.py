...
log_colors = {"DEBUG": "cyan", "INFO": "green", "WARNING": "red", "ERROR": "bold_red", "CRITICAL": "bg_red"}


# logger = logging.getLogger("gunicorn.root")
# -*- coding: utf-8 -
#
...
CONFIG_DEFAULTS = dict(
    ...
    disable_existing_loggers=False,
    loggers={
        "gunicorn.root": {"level": "DEBUG", "handlers": ["console"]},
        "gunicorn.error": {
            "level": "INFO",
            "handlers": ["error_console"],
            "propagate": True,
    ...
    formatters={
        "generic": {
            "format": "%(asctime)s  %(cyan)s(%(module)s-%(funcName)s:%(lineno)d) %(bold_white)s>>%(reset)s %(message)s",
            "datefmt": "[%Y-%m-%d %H:%M:%S %z]",
